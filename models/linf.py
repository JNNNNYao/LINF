...
class LINF(nn.Module):
    ...
    def query_log_p(self, inp, feat, coord, cell, gt):
        gt_residual = gt - F.grid_sample(inp, coord.flip(-1), mode='bilinear',\
        ...
        z, log_p = self.imnet(gt_residual.permute(0, 2, 3, 1).contiguous().view(bs * w * h, -1), affine_info.permute(0, 2, 3, 1).contiguous().view(bs * w * h, -1))
    ...
    def rgb(self, inp, coord, cell, temperature=0):
        feat = self.gen_feat(inp)
    ...
    def forward(self, op, inp=None, feat=None, coord=None, cell=None, gt=None, temperature=0):
        ...
        if op == "rgb":
            return self.rgb(inp, coord, cell, temperature)
        if op == "gen_feat":
